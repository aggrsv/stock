...
func Laster(symbol string) (*entity.Stock, error) {
	resp, err := utils.Get(fmt.Sprintf(entity.APILaster, symbol))
	if err != nil {
		return nil, err
	}
	enc := mahonia.NewDecoder("gbk")
	elements := strings.Split(enc.ConvertString(string(resp)), "~")
	if len(elements) < 49 {
		return nil, errors.New("stock symbol not found")
	}
	stock := &entity.Stock{
		// 未知
		Unknow: elements[0],
